...
public class JokeTeller {
    ...
    private static final String defaultJoke = "lol";

    public JokeTeller() {

        //jokeFilePath = FileSystems.getDefault().getPath(jokeFileName);

        random = new Random();
        if (jokeList != null)
            numOfJokes = jokeList.length;
    }

    public String getJoke() {
        numOfJokes = jokeList.length;
        return numOfJokes > 0 ? jokeList[random.nextInt(numOfJokes)] : defaultJoke;

    }


    private static final String[] jokeList = { "If you have anything staged, commit now or stash forever",
            ...
            "git-svn: now you have tw o problems.",
